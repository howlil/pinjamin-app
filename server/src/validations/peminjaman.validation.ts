...
export const peminjamanSchema = z.object({
  ...
  tanggal_mulai: z.string().regex(/^\d{2}-\d{2}-\d{4}$/, 'Format tanggal mulai tidak valid'),
  tanggal_selesai: z.string().regex(/^\d{2}-\d{2}-\d{4}$/, 'Format tanggal selesai tidak valid'),
...
export const peminjamanUpdateSchema = z.object({
  ...
  tanggal_mulai: z.string().regex(/^\d{2}-\d{2}-\d{4}$/, 'Format tanggal mulai tidak valid').optional(),
  tanggal_selesai: z.string().regex(/^\d{2}-\d{2}-\d{4}$/, 'Format tanggal selesai tidak valid').optional(),
