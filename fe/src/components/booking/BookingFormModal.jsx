...
const BookingFormModal = ({ isOpen, onClose, roomName, buildingData, roomData }) => {
    ...
    // Extract buildingId from buildingData, roomData, or URL parameter as fallback
    const buildingId = buildingData?.id || roomData?.id || roomId;
    ...
    if (!buildingId) {
        console.warn('BookingFormModal: No buildingId provided. BuildingData:', buildingData, 'RoomData:', roomData, 'RoomId:', roomId);
    }

    // Check if user is authenticated and not admin
    const canMakeBooking = isAuthenticated && user?.role === 'BORROWER';

    // Don't render modal if no buildingId is available
        ...
        <Modal isOpen={isOpen} onClose={onClose} size="md" isCentered>
            ...
            >
                ...
                <ModalBody pb={6}>
                    ...
                    {!buildingId && (
                        <VStack spacing={4} textAlign="center" py={8}>
                            ...
                            <Text fontSize="sm" color="gray.600">
                                Tidak dapat memuat data gedung. Silakan refresh halaman atau coba lagi nanti.
                            </Text>
                            <Button
                                variant="outline"
                                colorScheme="red"
                    ...
                    {buildingId && isAuthenticated && user?.role === 'ADMIN' && (
                        <VStack spacing={4} textAlign="center" py={8}>
                            <Text fontSize="lg" fontWeight="semibold" color={COLORS.black}>
                                Akses Dibatasi
                            </Text>
                    ...
                    {/* Show booking form if authenticated as borrower and buildingId exists */}
                    {buildingId && canMakeBooking && (
