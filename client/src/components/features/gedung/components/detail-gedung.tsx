...
import { Gedungs, TipeGedung, PenanggungJawabGedung } from "@/interfaces/IGedung";
...
export default function DetailGedung({ data }: DetailGedungProps) {
  ...
  const [activeTab, setActiveTab] = useState<'info' | 'facilities' | 'contact'>('info');
  ...
  return (
    <div className="bg-[#bfffac1d] p-4 rounded-lg">
      <div className="relative mb-4 overflow-hidden rounded-lg h-48 bg-gray-100">
        <img 
          src={`/assets/images/${data.foto_gedung}`} 
        ...
        <div className="absolute left-0 right-0 bottom-0 p-3 text-white">
          ...
          <div className="flex items-center space-x-2 mt-1">
            ...
            <span className="text-[#FCA129] font-semibold ml-1">{formatRupiah(data.harga_sewa)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex border-b border-gray-200 mb-4">
        <button
          ...
          className={`px-3 py-2 text-sm font-medium relative ${activeTab === 'info' ? 'text-[#749C73] border-b-2 border-[#749C73]' : 'text-gray-500'}`}
        >
          Informasi
        </button>
        <button
          onClick={() => setActiveTab('facilities')}
          className={`px-3 py-2 text-sm font-medium relative ${activeTab === 'facilities' ? 'text-[#749C73] border-b-2 border-[#749C73]' : 'text-gray-500'}`}
        >
          Fasilitas
        </button>
        <button
          onClick={() => setActiveTab('contact')}
          className={`px-3 py-2 text-sm font-medium relative ${activeTab === 'contact' ? 'text-[#749C73] border-b-2 border-[#749C73]' : 'text-gray-500'}`}
        >
          Kontak
        </button>
      </div>
      ...
      <div className="min-h-[200px]">
        <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="bg-white/20 backdrop-blur-md p-3 rounded-lg border mb-4">
                <div className="grid grid-cols-3 gap-2 text-sm">
                  <div>
                    ...
                    <div className="font-medium">{data.TipeGedung?.nama_tipe_gedung || "-"}</div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="font-medium text-[#749C73]">{formatRupiah(data.harga_sewa)}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white/20 backdrop-blur-md p-3 rounded-lg border mb-4">
                ...
                <div className="flex items-start">
                  <svg className="w-4 h-4 text-[#749C73] mt-0.5 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {activeTab === 'facilities' && (
            ...
            >
              {data.FasilitasGedung && data.FasilitasGedung.map((fasilitas: FasilitasGedung, index) => (
                <motion.div 
                  key={fasilitas.id}
                ...
                >
                  ...
                  <span className="text-xs text-gray-700">{fasilitas.nama_fasilitas}</span>
                </motion.div>
              ))}
            </motion.div>
          )}
          
          {activeTab === 'contact' && data.penganggung_jawab_gedung && data.penganggung_jawab_gedung.length > 0 && (
            <motion.div
              key="contact"
              initial={{ opacity: 0, y: 5 }}
              animate={{ opacity: 1, y: 0 }}
              ...
              className="space-y-2"
            >
              {data.penganggung_jawab_gedung.map((pj: PenanggungJawabGedung, index) => (
                <motion.div 
                  key={pj.id}
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ 
                ...
                >
                  <div className="flex items-center">
                    ...
                    <div>
                      <div className="font-medium text-sm">{pj.nama_penangguang_jawab}</div>
