...
export default function CalendarGedung({
  data,
  onBookingClick,
}: CalendarGedungProps) {
      ...
      <div className="bg-white rounded-lg overflow-hidden  border ">
        <div className="grid grid-cols-7">
          {daysOfWeek.map((day, index) => (
            <div
              key={day}
              className="text-center py-2 text-xs font-medium text-gray-500 border-b border-gray-100"
            >
              {day}
            </div>
          ))}
        ...
        <div className="grid grid-cols-7">
          {calendarDays.map((dayObj, index) => (
            <div
              key={index}
              className={`relative ${
                index % 7 !== 0 ? "border-l border-gray-50" : ""
              } ${index >= 7 ? "border-t border-gray-50" : ""}`}
                ...
                <div className="py-1 px-1 h-14">
                      ...
                      className={`
                        ...
                        ${
                          selectedDate === dayObj.date
                            ? "ring-2 ring-[#749C73] ring-opacity-50"
                            : ""
                        }
                  ...
                  {dayObj.bookings.length > 0 && hoveredDay === dayObj.day && (
                    ...
                    >
                      {dayObj.bookings.map((booking, bIdx) => (
                        ...
                        >
                          <div
                            className={`h-2 w-2 rounded-full mr-1.5 ${getStatusIndicator(
                  ...
                  {dayObj.bookings.length > 0 && hoveredDay !== dayObj.day && (
                    <div className="flex justify-center mt-1">
                      {dayObj.bookings
                        .slice(0, Math.min(3, dayObj.bookings.length))
                        .map((booking, idx) => (
                          <div
                            key={idx}
                            className={`h-1.5 w-1.5 rounded-full mx-0.5 ${getStatusIndicator(
                              booking.status_peminjaman
                            )}`}
                          ></div>
                        ))}
